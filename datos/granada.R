...
for (i in c("PALACIO DE CONGRESOS","MOTRIL") ) {
    granada_congresos<- granada[granada$estacion==i,]
#granada_congresos<- granada[granada$estacion=="MOTRIL",]
#granada_congresos<- granada[granada$estacion=="GRANADA-NORTE",]
#granada_congresos<- granada[granada$estacion=="PASEOS UNIVERSITARIOS",]
    ...
    granada_congresos_error$var2 = as.numeric(granada_congresos_error$year) + 15
    y_labels = levels(granada_congresos_error$year)
    y_breaks = seq_along(y_labels) + 15
    
    p2 = ggplot(granada_congresos_error, aes(x=number_day, y=var2, label=month, angle=ang, hjust=hjust,fill=V1)) +
